...
namespace EDSDK
{
    ...
    public class EDSDKLib
    {
        ...
        public static uint EdsGetPropertyData(IntPtr inRef, uint inPropertyID, int inParam,
             out EDSDKLib.EdsTime outPropertyData)
        {
            int size = Marshal.SizeOf(typeof(EDSDKLib.EdsTime));
            IntPtr ptr = Marshal.AllocHGlobal(size);
            uint err = EdsGetPropertyData(inRef, inPropertyID, inParam, size, ptr);

            outPropertyData = (EDSDKLib.EdsTime)Marshal.PtrToStructure(ptr, typeof(EDSDKLib.EdsTime));
            Marshal.FreeHGlobal(ptr);
            return err;
        }
